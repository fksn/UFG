...
#include <string.h>
#include <stdlib.h>
//Struct para guardar dados do cliente
struct cliente
{
    char NOME[32], LOGIN[32], SENHA[32],  ENDERECO[64];
}typedef CLIENTE;
//struct para armazenar as compras
struct compra
{
    char NOME[32], ENDERECO[64];
    int * COD_ITENS;
    int * QUANT_ITENS;
    int QUANT_PROD;
    double P_Compra;
...
fila *p, *q, *ini, *fim;
//função para ver se o arquivo de save já existe
void Inicio();
//função para transformar em maiusculo
void MAIUSCULA(char * N);
//função para onde é feita a pesquisa dos produtos
void BuscaProduto();
//função para listar todos os produtos cadastrados
void ListarProdutos();
//função para listar todas as compras já feitas
void ListarCompras();
//função para abrir o arquivo e armazenalos nas structs
int Abre_Arq();
//função para armazenar as structs no arquivo de forma binaria
int Salvar_Arq();
//função em busca binaria para ver se os codigos já foram usados
int binarySearch(int x, int low, int high);
//função para iniciar login de forma binaria
int VerificaBinario(char * N, int low, int high);
//função para produrar o nome do produto de forma binaria
...
//função para realizar loginj e direcionar para a realização da compra
void Login();
//função para "andar" com a fila, mostrando as compras na ordem correta
void VerFila();
//função para o funcionario apagar todos os dados dop sistema
void Reset();
...
int main()
{
    ...
    while (Sel != 2)
    {
        ...
        else
        {
            ...
            {
                system("tput reset"); //Limpa o terminal
                printf("Escolha o que deseja fazer:\n1 - Cadastrar novo produto\n2 - Pesquisar produto\n3 - Fila de compras\n4 - Listal todos os produtos\n5 - Historico de compras geral\n6 - Resetar arquivos\n7 - Voltar\n");
                scanf("%d", &op);
                while (op != 7)
                {
                    if (op == 1)
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            system("tput reset"); //Limpa o terminal
                                            printf("!!! ISSO VAI APAGAR TODOS OS DADOS !!!\n");
...
int Abre_Arq()
{
    ...
    {
        fread(&(C[i].NOME), sizeof(char), 32, arq);
        fread(&(C[i].ENDERECO), sizeof(char), 64, arq);
...
int Salvar_Arq()
{
    ...
    for ( i = 0; i < NUM_COMPRA-1; i++)
    {
        fwrite(&(C[i].NOME), sizeof(char), 32, arq);
        fwrite(&(C[i].ENDERECO), sizeof(char), 64, arq);
...
void RealizarCompra(int X)
{
    ...
    strcpy(C[NUM_COMPRA-1].NOME, USER[X].NOME);
    strcpy(C[NUM_COMPRA-1].ENDERECO, USER[X].ENDERECO);
    ...
    while (strcmp(N, "-1") != 0)
    {
        ...
        else
        {
            ...
            C[NUM_COMPRA-1].COD_ITENS[cont] = ITEM[cod].COD;
...
void ListarProdutos()
{
    ...
    for (i = 0; i < NUM_PROD-1; i++)
    {
        printf("Indice: %d, Nome: %s, Marca: %s, Codigo: %d, Quantidade: %d, Preço de compra: %.2lf, Preço de venda: %.2lf\n", i, ITEM[i].NOME, ITEM[i].MARCA, ITEM[i].COD, ITEM[i].QUANT, ITEM[i].P_COMPRA, ITEM[i].P_VENDA);
    }
    printf("Para retornar digite 1: ");
    scanf("%d", &aux);
    while (aux != 1)
    {
        printf("Para retornar digite 1: ");
...
void ListarCompras()
{
    ...
    for (i = 0; i < NUM_COMPRA-1; i++)
    {
        printf("Indice: %d\nNome do cliente: %s, Preço: %.2lf\n", i, C[i].NOME, C[i].P_Compra);
        for (j = 0; j < C[i].QUANT_PROD; j++)
        {
            aux = C[i].COD_ITENS[j];
            printf("Item: %d, Quantidade: %d\n", C[i].COD_ITENS[j], C[i].QUANT_ITENS[j]);
        }
        printf("\n");
    }
    printf("Para retornar digite 1: ");
...
void VerFila()
{
    ...
    {
        printf("Produto: %d | Quantidade: %d\n", C[X].COD_ITENS[i], C[X].QUANT_ITENS[i]);
    }
    p = p->link;
    ini = p;
    NUM_FILA--;
