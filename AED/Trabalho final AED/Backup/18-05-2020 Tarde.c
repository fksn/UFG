...
void OrdenaNovo(PRODUTO * ITEM, char N[32]);
void BuscaProduto(PRODUTO * ITEM);
...
int VerificaBinario(CLIENTE * USER, char * N, int low, int high);
PRODUTO * NovoProduto(PRODUTO * ITEM);
CLIENTE * NovoUsuario(CLIENTE * USER);
COMPRA * RealizarCompra(CLIENTE * USER, PRODUTO * ITEM, COMPRA * C);

int NUM_PROD, NUM_USER = 1, NUM_COMPRA = 1;
...
int main()
{
    ...
    USER = (CLIENTE *) malloc(2 * sizeof(CLIENTE));
        ...
        if (Sel == 0)
        {
            ...
            while (Sel != 2)
            {
                if (Sel == 0)
                {
                    ...
                    scanf("%*c%[^\n]", N);
                    MAIUSCULA(N);
                    X = VerificaBinario(USER, N, 0, NUM_USER-1);
                    printf("X = %d", X);
                    while (X == -1)
                    {
